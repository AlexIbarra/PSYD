...
static uint32 loop_ms = 0; // almacena el nÃºmero de iteraciones para retardar 1 ms
static uint32 loop_s = 0; // almacena el nÃºmero de iteraciones para retardar 1 s


static void sw_delay_init( void ) {
    uint32 i;
    /* Mide la duracion de un millon de iteraciones */
    timer3_start(); // la resoluciÃ³n del temporizador es 100us
    for( i=1000000; i; i-- );
    loop_s = ((uint64)1000000*10000)/timer3_stop(); // Regla de tres: si 10^6 iteraciones tardan n*100us, en 1s se haran 10^6 / (nÃ— 0,0001) iteraciones
    loop_ms = loop_s / 1000; // en 1 ms se harÃ¡n 1000 veces menos iteraciones que en 1 s
}
...
void timer0_open_tick( void (*isr)(void), uint16 tps ) {
    ...
    I_ISPC = BIT_TIMER0; // borra flag de interrupciÃ³n pendiente por interrupciones del timer 0
    INTMSK &= ~( BIT_GLOBAL | BIT_TIMER0 ); // desenmascara globalmente interrupciones e interrupciones del timer 0
    if( tps > 0 && tps <= 10 ) {
        /* programa el T0 con resoluciÃ³n de 25 us (40 KHz) */
        ...
        /* programa el T0 con resoluciÃ³n de 2,5 us (400 KHz) */
...
void timer0_open_ms( void (*isr)(void), uint16 ms, uint8 mode ) {
    ...
    I_ISPC = BIT_TIMER0; // borra flag de interrupciÃ³n pendiente por interrupciones del timer 0
    INTMSK &= ~( BIT_GLOBAL | BIT_TIMER0 ); // desenmascara globalmente interrupciones e interrupciones del timer 0
    /* programa el T0 con resoluciÃ³n de 100us */
